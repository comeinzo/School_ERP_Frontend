...
//             return <CreateClass stats={stats} />;
//         }
//     };
...
import { Box, Tabs, Tab } from '@mui/material'; // Import MUI components
...
const AdminAddClassLayout = () => {
  ...
  // --- State for MUI Tabs (uses a number index, 0 for 'View', 1 for 'Add') ---
  ...
  useEffect(() => {
    // Your existing useEffects are fine
    fetchUserData();
    const savedSidebarState = localStorage.getItem('sidebarCollapsed');
    if (savedSidebarState) {
      setSidebarCollapsed(JSON.parse(savedSidebarState));
  ...
  // Your fetchUserData, handleLogout, etc. functions remain the same...
  const fetchUserData = async () => {
    setLoading(true);
    try {
      const userData = await authService.getCurrentUser();
...
  };
  ...
  // --- Updated rendering logic based on tab index ---
  const renderContent = () => {
    switch (activeTab) {
      case 0:
        return <ViewClasses stats={stats} />;
    ...
    <div className="admin-layout">
      ...
      <div className={`admin-layout-main ${sidebarCollapsed ? 'expanded' : ''}`}>
        ...
        <main className="admin-content">
          ...
          <div className="class-content-container" style={{ paddingTop: '24px' }}>
            {renderContent()}
          </div>
        </main>
      </div>
