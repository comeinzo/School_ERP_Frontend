...
const AdminAddStudentLayout = () => {
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState(0); // Single state for managing tabs
    ...
    <div className="admin-layout">
      ...
      <div className={`admin-layout-main ${sidebarCollapsed ? 'expanded' : ''}`}>
        ...
        <main className="admin-content">
          <div className="class-management-header">
            <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
            <Tabs value={activeTab} onChange={handleTabChange} indicatorColor="primary" variant="fullWidth">
               <Tab label="Student Admission" />
...
// import Header from '../../components/Header/Header';
// import StudentAdmissionForm from '../../components/Students/StudentsForm';
// import StudentBulkAdmission from '../../components/Students/StudentBulkAdmission'
// import { Box, Tabs, Tab, Typography, Snackbar, Alert, CircularProgress } from "@mui/material";
...
//   const navigate = useNavigate();
//   const location = useLocation();
//   const sidebarItems = getSidebarItems({}); // Pass empty stats or real stats if available
...
//     try {
//       setFormLoading(true);
//       const [classes, nextNumber] = await Promise.all([
//         classService.getClasses(),
//         studentService.getNextAdmissionNumber()
//       ]);
//       setAllClasses(classes);
//       setAdmissionNumber(nextNumber); // Simplified: assumes API returns a string
//     } catch (error) {
//       console.error('Failed to fetch student form data:', error);
//       setSnackbar({ open: true, message: 'Failed to load initial form data.', severity: 'error' });
//       setAdmissionNumber('ERROR'); // Set a clear error state
//     } finally {
//       setFormLoading(false);
//     }
//   };
...
//       // Append all fields from the form data object to the FormData instance
//       Object.entries(data).forEach(([key, value]) => {
//         if (key === 'photo') {
//           // Handle the photo file specifically
//           if (value instanceof File) {
//             formData.append('photo', value, value.name);
//           }
//         } else {
//           // Handle all other fields
//           // Convert null/undefined to an empty string to avoid issues
//           const stringValue = value !== null && value !== undefined ? String(value) : '';
//           formData.append(key, stringValue);
//         }
//       });
...
//       // Call the service with the correctly formatted FormData object
//       const response = await studentService.addStudent(formData);
      
//       setSnackbar({
//         open: true,
//         message: `Student "${response.student_name}" admitted successfully!`,
//         severity: 'success'
//       });
      
//       // Refresh form data (like the next admission number) after success
//       await fetchStudentFormData();
      
//     } catch (error) {
//       console.error('Failed to add student:', error);
//       const errorMessage = error.response?.data?.detail || 'Failed to submit student admission.';
//       setSnackbar({
//         open: true,
//         message: errorMessage,
//         severity: 'error'
...
//   // The single, corrected renderContent function
//   const renderContent = () => {
//     switch (activeTab) {
//       case 0:
//         return (
...
//         collapsed={sidebarCollapsed}
//         onToggle={toggleSidebar}
//         activeSection={'students'} // Hardcoded as this is the student layout
//         user={user}
//         onLogout={handleLogout}
//       />
//       <div className={`admin-layout-main ${sidebarCollapsed ? 'expanded' : ''}`}>
